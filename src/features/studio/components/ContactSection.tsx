...
({ icon, label, value, href, isVisible, delay }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="text-sm text-cyber">{label}</p>
...
export const ContactSection: React.FC = () => {
  ...
  return (
    <section id="contact" className="bg-black text-white py-20 sm:py-28 scroll-mt-20 relative">
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="grid lg:grid-cols-2 gap-24 md:gap-32 items-start">
          ...
          <div className="space-y-12">
            <h3 
              className={`text-4xl font-serif text-transparent bg-clip-text bg-gradient-to-b from-white to-cyan-200/80 text-center fade-in-up ${isVisible ? 'visible' : ''}`}
              style={{ 
              ...
              <ul className="space-y-4 text-sm max-w-sm mx-auto">
                {BUSINESS_HOURS.map((item, i) => (
                  <li key={item.day} className="flex justify-between border-b border-white/10 pb-3 last:border-none">
                    ...
                    <span className={`font-semibold ${item.hours === 'Closed' ? 'text-white/50' : 'text-white'}`}>{item.hours}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="text-center">
            <h3 
                className={`text-4xl font-serif text-transparent bg-clip-text bg-gradient-to-b from-white to-cyan-200/80 mb-6 fade-in-up ${isVisible ? 'visible' : ''}`}
                style={{ 
                  transitionDelay: '400ms',
                  textShadow: isVisible ? 'var(--cyan-glow-intense)' : 'var(--cyan-glow-subtle)',
                }}
            ...
            <form onSubmit={handleSubscribe} className={`space-y-4 w-full max-w-sm mx-auto fade-in-up ${isVisible ? 'visible' : ''}`} style={{ transitionDelay: '600ms' }}>
              <div className="flex flex-col sm:flex-row gap-3">
                <input
                  ...
                  className="flex-grow w-full px-5 py-3.5 bg-transparent border border-cyber/30 rounded-lg text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-cyber focus:border-cyber transition-all duration-300"
              ...
              {status === 'success' && <p className="text-sm text-green-400 mt-2">Thank you for subscribing!</p>}
            </form>
            
            <div className={`mt-12 fade-in-up ${isVisible ? 'visible' : ''}`} style={{ transitionDelay: '700ms' }}>
              <p className="text-sm text-white/50">
                For other inquiries, please email us at <a href={`mailto:${COMPANY_INFO.contact.email}`} className="text-cyber hover:underline">{COMPANY_INFO.contact.email}</a>.
              </p>
            </div>
          </div>
