...
const TimeSlot: React.FC<{ time: string; isSelected: boolean; onSelect: () => void }> = ({
  time,
  isSelected,
  onSelect,
}) => (
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            {practitioners.map((p) => (
              <button
                key={p.name}
                onClick={() => {
                  setSelection({ ...selection, practitioner: p });
                  handleNext();
                }}
                className={cn(
        ...
        return (
          <div className='text-center bg-deep-900/50 p-8 rounded-2xl border border-white/10'>
            ...
            <div className='text-left max-w-sm mx-auto space-y-3 text-gray-300'>
              <p>
                <strong className='text-white'>Service:</strong> {selection.service?.name}
              </p>
              <p>
                <strong className='text-white'>Practitioner:</strong> {selection.practitioner?.name}
              </p>
              <p>
                <strong className='text-white'>Date:</strong> {selection.date}
              </p>
              <p>
                <strong className='text-white'>Time:</strong> {selection.time}
              </p>
            </div>
          </div>
        );
              ...
              <div className='flex flex-col items-center z-10'>
                <div
                  className={cn(
                    ...
                    index <= currentStep
                      ? 'bg-cyber border-cyber text-deep-950'
                      : 'bg-deep-800 border-white/20 text-white'
                  )}
                >
                  <step.icon size={20} />
                </div>
      ...
      {steps[currentStep].id !== 'confirm' && (
        <div className='flex justify-between items-center mt-8'>
          ...
          <Button
            ...
            disabled={
              !selection.service ||
              (currentStep === 1 && !selection.practitioner) ||
              (currentStep === 2 && (!selection.date || !selection.time))
            }
          >
